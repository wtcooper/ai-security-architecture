...
Band assignment is computed from each component's own `category` / `subcategory` in `components.yaml` (see `src/lib/bands.ts`) — no component is placed by hand. The last column shows where Google's SAIF map drew the same component, so the divergences are explicit.

| Component | CoSAI category / subcategory | Our band | SAIF band | |
| --- | --- | --- | --- | --- |
| Data Sources | `componentsInfrastructure` / `componentsData` | **data** | data |  |
| Data Filtering and Processing | `componentsInfrastructure` / `componentsData` | **data** | data |  |
| Training Data | `componentsInfrastructure` / `componentsData` | **data** | data |  |
| Data Storage Infrastructure | `componentsInfrastructure` / `componentsData` | **data** | infrastructure | moved |
| Model Frameworks and Code | `componentsModel` / `componentsModelTraining` | **model** | infrastructure | moved |
| Model Evaluation | `componentsModel` / `componentsModelTraining` | **model** | infrastructure | moved |
| Training and Tuning | `componentsModel` / `componentsModelTraining` | **model** | infrastructure | moved |
| Model Storage | `componentsInfrastructure` / `componentsModelDeployment` | **infrastructure** | infrastructure |  |
| Model Serving Infrastructure | `componentsInfrastructure` / `componentsModelDeployment` | **infrastructure** | infrastructure |  |
...
| Application Output Handling | `componentsApplication` / `componentsApplicationCore` | **application** | model | moved |
| Application Input Handling | `componentsApplication` / `componentsApplicationCore` | **application** | model | moved |
| Agent Reasoning Core | `componentsApplication` / `componentsAgent` | **application** | application |  |
| Orchestration Output Handling | `componentsModel` / `componentsOrchestration` | **model** | application | moved |
| Orchestration Input Handling | `componentsModel` / `componentsOrchestration` | **model** | application | moved |
| External Tools and Services | `componentsModel` / `componentsOrchestration` | **model** | application | moved |
| Model Memory | `componentsModel` / `componentsOrchestration` | **model** | application | moved |
| Retrieval Augmented Generation & Content | `componentsModel` / `componentsOrchestration` | **model** | application | moved |
...
23 components, all drawn on the map. 11 sit in a different band than SAIF used, because CoSAI classifies them differently:

- **Data Storage Infrastructure** — SAIF: infrastructure; CoSAI: `componentsInfrastructure`/`componentsData` → data.
- **Model Frameworks and Code** — SAIF: infrastructure; CoSAI: `componentsModel`/`componentsModelTraining` → model.
- **Model Evaluation** — SAIF: infrastructure; CoSAI: `componentsModel`/`componentsModelTraining` → model.
- **Training and Tuning** — SAIF: infrastructure; CoSAI: `componentsModel`/`componentsModelTraining` → model.
- **Application Output Handling** — SAIF: model; CoSAI: `componentsApplication`/`componentsApplicationCore` → application.
- **Application Input Handling** — SAIF: model; CoSAI: `componentsApplication`/`componentsApplicationCore` → application.
- **Orchestration Output Handling** — SAIF: application; CoSAI: `componentsModel`/`componentsOrchestration` → model.
- **Orchestration Input Handling** — SAIF: application; CoSAI: `componentsModel`/`componentsOrchestration` → model.
- **External Tools and Services** — SAIF: application; CoSAI: `componentsModel`/`componentsOrchestration` → model.
- **Model Memory** — SAIF: application; CoSAI: `componentsModel`/`componentsOrchestration` → model.
- **Retrieval Augmented Generation & Content** — SAIF: application; CoSAI: `componentsModel`/`componentsOrchestration` → model.

## 2. Flows

Every arrow on the map is an edge declared in `components.yaml`, and every declared edge is drawn. The build compares the two sets and fails on any addition, omission or reversal, so the arrows cannot drift from CoSAI.
...
| Orchestration Output Handling | The Model |
...
| The Model | Orchestration Input Handling |

...
import type { Mitigation, Surface } from "../../src/lib/types";
...
export async function loadMitigations(root: string) {
  const profile = parse(await readFile(join(root, "data/overlay/mitigations.yaml"), "utf8")) as {
    attribution: string; surfaces: Surface[]; mitigations: Mitigation[];
  };
  const { sources } = parse(await readFile(join(root, "data/mitre/sources.yaml"), "utf8")) as { sources: Source[] };
  const upstream = new Map<string, Pick<Mitigation, "title" | "description" | "origin">>();
  for (const source of sources) {
    ...
    if (source.framework === "MITRE D3FEND") {
      ...
      const atlas = parse(raw) as { collection: { version: string }; mitigations: Record<string, AtlasMitigation> };
  ...
  const mitigations = profile.mitigations.map((c): Mitigation => {
  ...
  const ids = new Set(mitigations.map((c) => c.id));
  for (const [old, rule] of Object.entries(migration.mappings)) {
    if (!old.startsWith("capability") || rule.targets.some((id) => !ids.has(id))) {
      throw new Error(`Invalid mitigation migration: ${old}`);
    }
  }
  return { ...profile, mitigations, aliases: Object.fromEntries(Object.entries(migration.mappings).map(([id, rule]) => [id, rule.targets])) };
}

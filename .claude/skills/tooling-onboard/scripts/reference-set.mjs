#!/usr/bin/env node
// Prints the reference control set of an architecture — the mitigations and risks pinned on
...
if (!id) {
  for (const a of d.archetypes) console.log(`${a.id}  ${a.surface}  ${a.title}  (${a.mitigations.length} mitigations, ${a.risks.length} risks)`);
  process.exit(0);
}
const a = d.archetypes.find((x) => x.id === id);
if (!a) { console.error(`unknown architecture ${id}`); process.exit(1); }
const cap = new Map(d.mitigations.map((c) => [c.id, c]));
const risk = new Map(d.risks.map((r) => [r.id, r]));
const one = (p) => (Array.isArray(p) ? p.flat().join(" ") : String(p ?? "")).replace(/\s+/g, " ").slice(0, 150);
console.log(`# ${a.title} (${a.id})\n\n## Pinned mitigations (${a.mitigations.length})`);
for (const c of a.mitigations) console.log(`- ${cap.get(c)?.title} (${c}): ${one(cap.get(c)?.description)}`);

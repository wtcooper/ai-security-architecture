...
import { CellDetail, CellTile, cellTitle, docsUrlFor } from "./shared";

export function GridView({ tools, groups, overlay }: { tools: Tool[]; groups: RowGroup[]; overlay: boolean }) {
  const cols = columnGroups(tools);
  const [picked, setPicked] = useState<{ tool: Tool; row: Row } | null>(null);
  const span = tools.length + 1;
  return (
    <div className="space-y-3">
      <div className="max-h-[75vh] overflow-auto rounded-xl border border-line bg-paper">
        <table className="border-separate border-spacing-0 text-[12px]">
          ...
          <tbody>
            {groups.map((group) => (
              <GroupRows key={group.id} group={group} cols={cols} span={span} picked={picked} onPick={setPicked} overlay={overlay} />
            ))}
          </tbody>
        </table>
      </div>

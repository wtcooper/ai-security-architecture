...
id and exactly one MITRE parent. Under Generative AI Guardrails (AML.M0020) sit input screening
guardrails and output content safety guardrails, the two directions of the same gateway crossing,
sensitive-data redaction at the DLP point, and retrieval grounding checks on the retrieval path.
A specialisation is a distinct place or direction in the data flow; both guardrail directions keep
"Guardrails" in their name because one product usually provides both. Specialisations are the pin
unit where they exist; they inherit the parent's controls,

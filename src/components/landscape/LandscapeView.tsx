...
/**
 * The landscape view on the Reference architectures page: the framing's provenance line, the
 * legend, the security-domains drawing itself, and the selected capability's full record beneath
 * it.
 */
...
export function LandscapeView({ view, surface, aiOnly }: { view: View; surface: string | null; aiOnly: boolean }) {
  ...
  return (
    <div className="mt-4">
      <Legend overlay={overlay} shown={tiles.length} aiOnly={aiOnly} />
      <div className="mt-3">
        <DomainPoster {...props} />
      </div>
      <p className="mt-2 text-xs text-ink-3">
        Each MITRE parent has one home — an authored judgement, not the framework&rsquo;s. Its specialisations roll up into it, as on the Capabilities page. Framing after{" "}
...
function Legend({ overlay, shown, aiOnly }: { overlay: boolean; shown: number; aiOnly: boolean }) {
  return (
